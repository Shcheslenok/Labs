...
% Last Modified by GUIDE v2.5 14-Dec-2017 00:13:54
...
step = get(handles.slider3,'Value');
x = -5:step:5;

x1 = x(x <=0);
x2 = x(x > 0);
y1 = (3+sin(2*x1).^2)./(1+ cos(x1).^2);
y2 = 2*x2 + sin(2*x2)/(3+x2);
y = [y1 , y2];

z1 = (3*x1.^2)./(1+x1.^2);
z2 = sqrt(1 + 2*x2./(exp(0.5*x2) + x2.^2));
z = [z1 , z2];
...
if (get(handles.listbox2, 'Value') == 1)
    ...
    plot(handles.axes1,x,z,'Color', color);
end
